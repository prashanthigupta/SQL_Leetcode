...
The table holds all taxi trips. Each trip has a unique Id, while Client_Id and Driver_Id 
are foreign keys to the Users_Id at the Users table.
...
round(sum(left(t.status,9)="cancelled")
      /count(t.id)
...
# Second Method here: 这里不能是else 0  不然 count肯定是错的呀
count(CASE WHEN left(t.status,9)="cancelled" THEN 1 ELSE NULL END)