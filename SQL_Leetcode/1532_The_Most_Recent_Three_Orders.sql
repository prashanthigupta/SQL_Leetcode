1532_The_Most_Recent_Three_Orders.sql
...
from
		...
		row_number() over (partition by c.customer_id 
												order by o.order_date desc) as ranki
		from orders o
		inner join customers c on c.customer_id=o.customer_id 
		) as t

where t.ranki<=3
order by t.customer_name, t.customer_id, t.order_date desc
# 这里其实row number（） 是最好的，
# rank（）的话 确实考虑了 一个人同一天order 了几单的情况，
# 但题目的意思感觉大体还是指order 而不是日期本身