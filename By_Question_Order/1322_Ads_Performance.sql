...
ad_id,
ifnull(round(
    100*sum(action="Clicked") / (sum(action="Clicked") + sum(action="Viewed"))
    ,2),0) as ctr
...
# 这个办法比较妙，相当于 avg（clicked） 毕竟分母的view 变成了0 也算了一次
...
case when sum(action="Clicked")+sum(action="Viewed")=0 then 0
         ...
         #Not good way: sum(action="Clicked") / sum(action!="Ignored") *100 
end
,2) as ctr
