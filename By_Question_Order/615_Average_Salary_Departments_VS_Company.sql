...
since there is only one employee in this department. 
...
select
left(temp.pay_date,7) as pay_month, 
temp.department_id,

case when temp.a_avg>temp.c_avg then "higher" 
     when temp.a_avg<temp.c_avg  then "lower"
     else "same"
end as comparison

from
(select 
s.id,s.employee_id,e.department_id, s.amount, s.pay_date,
avg(s.amount) over (partition by left(s.pay_date,7), e.department_id) as a_avg,
avg(s.amount) over (partition by left(s.pay_date,7)) as c_avg

from salary s
inner join employee e on e.employee_id=s.employee_id ) as temp

group by left(temp.pay_date,7), temp.department_id
